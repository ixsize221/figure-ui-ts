...
// Тут прописываем руками какой рендерер для какой фигуры. Можно автоматизировать(Рендер+ИмяКласса)
class ФабрикаРендеровФигур {
    static нужныйРендерер(фигура: Фигура): РендерФигуры {
        ...
        throw new Error(`Нет указаного рендерера для типа фигуры: ${фигура.constructor.name}`);
    }
}
...
фигуры.forEach(фигура => {
    const рендерер = ФабрикаРендеровФигур.нужныйРендерер(фигура);
