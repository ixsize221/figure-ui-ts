// https://github.com/Aksars/figure-ui-ts
// разархивировать, открыт в вс коде
// npm i
// npm run dev
...
function создатьHTML(прям: Прямоугольник, едeница: 'м' | 'см' = 'м'): string {
  ...
  if (едeница === 'м') {
    текстПериметр = `${периметр} м`
    текстПлощадь = `${площадь} м^2`
  }
  else {
    текстПериметр = `${периметр * 100} см`
    текстПлощадь = `${площадь * 10000} см^2
  `
  }

  return `
    <div class="result">
          <h1>Наш прямоугольник ${прям.height} х ${прям.width}</h1>          
          <h1>Периметр прямоугольника  = ${текстПериметр} </h1>
          <h1>Площадь прямоугольника  = ${текстПлощадь}  </h1>
    </div>`
}
...
// Привет. У меня есть проект на ????????
// ВОТ МОИ КЛАССЫ ФИГУР
// ...
// ВОТ МОЯ ФУКНЦИЯ КОТОРАЯ ИХ ВЫВОДИТ
// ...
// Я ХОЧУ ЧТОБ МОИ ФИГУРЫ ???????????
// ПОЛУЧИТЬ КОД И ПОПРОБОВАТЬ ВСТАВИТЬ В ПРОЕКТ